...
class MdcIsolationPropertyTest {
    ...
    /// 교집합은 당연히 공집합이다.
    @Property(tries = 100)
    void mdcEmpty_kvpAndMdcKeysAreDisjoint(@ForAll("docEvents") DocLogEvent taxonomy) {
        MDC.clear();
        try (var logs = CapturedLogs.of(MdcIsolationPropertyTest.class)) {
            LogEvents.event(LOGGER, taxonomy)
                    ...
                    .attr(LogFields.RPC_METHOD, "TestMethod")
    ...
    void mdcHasTraceId_kvpAndMdcKeysAreDisjoint(
            ...
            @ForAll("docEvents") DocLogEvent taxonomy) {
        MDC.clear();
    ...
    @Provide
    Arbitrary<DocLogEvent> docEvents() {
        return Arbitraries.of(DocLogEvent.values());
    }
}
