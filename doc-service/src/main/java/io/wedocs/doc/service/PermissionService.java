...
public class PermissionService {
    ...
    // 페이지 조상 트리 탐색의 방어적 깊이 상한 — 권한 상속 해석(여기)과 사이클 검사(PageTreeService)가
    // 공유하는 단일 선언(중복 제거). 정상 위키 깊이(수십 레벨)를 넉넉히 초과하며, 데이터 오염/미래
    // 버그의 안전망(secure-coding.md P2) — 정상 경로에선 도달 불가, 도달 시 fail-closed.
