...
class MessageIndependencePropertyTest {
    ...
    void allTaxonomyEntries_messageIsFixedFromTaxonomy() {
        ...
        try {
            for (GatewayLogEvent taxonomy : GatewayLogEvent.values()) {
                try (var logs = CapturedLogs.of(MessageIndependencePropertyTest.class)) {
                    ...
                    for (String requiredAttr : taxonomy.requiredAttributes()) {
                        if (requiredAttr.equals(LogFields.ERROR_TYPE)) {
                            emitter = emitter.errorType(GatewayErrorType.NO_TOKEN);
                        } else {
    ...
    Arbitrary<TaxonomyWithAttrs> taxonomyWithAttributes() {
        return Arbitraries.of(GatewayLogEvent.values()).flatMap(taxonomy -> {
    ...
    private Arbitrary<Map<String, String>> generateAttributes(GatewayLogEvent taxonomy) {
    ...
    private void emitWithAttributes(Logger logger, GatewayLogEvent taxonomy,
    ...
    record TaxonomyWithAttrs(GatewayLogEvent taxonomy, Map<String, String> attributes) {
    }
}
