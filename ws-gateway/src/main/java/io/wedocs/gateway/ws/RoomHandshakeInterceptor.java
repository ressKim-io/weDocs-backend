...
public class RoomHandshakeInterceptor implements HandshakeInterceptor {
    ...
    /// 검증된 RoomId(=docId)가 담기는 세션 attribute 키 — {@link HandshakeAttributes#ROOM_ATTRIBUTE}로 위임.
    public static final String ROOM_ATTRIBUTE = HandshakeAttributes.ROOM_ATTRIBUTE;

    /// 세션 attribute에서 RoomId를 타입 안전하게 읽는다 — {@link HandshakeAttributes#roomId}로 위임.
    public static Optional<RoomId> roomId(Map<String, Object> attributes) {
        return HandshakeAttributes.roomId(attributes);
    }
