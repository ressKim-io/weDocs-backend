...
/// 2. `wedocs.result` 또는 `wedocs.handshake.stage` = taxonomy가 선언한 값(둘 중 하나만)
...
public final class LogEvents {
    ...
    private final GatewayLogEvent taxonomy;
    private LoggingEventBuilder builder;
    private Throwable throwable;

    private LogEvents(Logger logger, GatewayLogEvent taxonomy) {
        this.taxonomy = taxonomy;
        this.builder = createBuilder(logger, taxonomy.level());
        prefill();
    }

    /// 이벤트 1건 시작 — event.name·wedocs.result|stage·상관 필드를 미리 채운 빌더를 돌려준다.
    public static LogEvents event(Logger logger, GatewayLogEvent taxonomy) {
    ...
    /// 오류 분류 — 열거 타입만 받는다(요구사항 4.3). 문자열 오버로드를 두지 않는 이유:
    /// 임의 문자열을 허용하면 대시보드가 열거값 밖의 값을 만난다.
    public LogEvents errorType(GatewayErrorType type) {
        if (type != null) {
    ...
    private void prefill() {
        ...
        // wedocs.result 또는 wedocs.handshake.stage (둘 중 하나만, null이면 생략)
        if (taxonomy.result() != null) {
            builder = builder.addKeyValue(LogFields.RESULT, taxonomy.result());
        }
        if (taxonomy.stage() != null) {
            builder = builder.addKeyValue(LogFields.HANDSHAKE_STAGE, taxonomy.stage());
        }
